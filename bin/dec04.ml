...
let all_directions () =
  Enum.cartesian_product (Enum.range (-1) ~until:1) (Enum.range (-1) ~until:1)
  |> Enum.filter_map (fun (x, y) ->
...
let check_x_mas field position =
  List.for_all (check_mas field position) diagonals

let positions input =
  let height = Array.length input in
  let width = String.length input.(0) in
  Enum.cartesian_product
    (Enum.range 0 ~until:(width - 1))
    (Enum.range 0 ~until:(height - 1))
  |> Enum.map pos_from_pair

let part1 input =
  positions input
  |> Enum.cartesian_product (all_directions ())
  |> Enum.map (fun (dir, pos) -> check_xmas input pos dir xmas)
  |> Enum.filter identity |> Enum.hard_count

let part2 input =
  positions input
  |> Enum.filter (fun pos -> get_2d input pos = Some 'A')
  |> Enum.map (check_x_mas input)
  |> Enum.filter identity |> Enum.hard_count
